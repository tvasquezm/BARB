export const WO_STATUSES = ['open', 'in_progress', 'done', 'closed'];
...
export function filterTickets(tickets, q) {
    return tickets.filter(t => {
        if (q.status && q.status !== 'all' && t.status !== q.status)
            return false;
        if (q.machineId && q.machineId !== 'all' && t.machineId !== q.machineId)
            return false;
        if (q.search) {
            const s = q.search.toLowerCase();
            if (!((t.title || '').toLowerCase().includes(s) || (t.description || '').toLowerCase().includes(s) || (t.id || '').toLowerCase().includes(s)))
                return false;
        }
        return true;
    });
}
export function getMachineHistory(machineId) {
